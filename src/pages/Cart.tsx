...
const CHECKOUT_STORAGE_KEY = "sumi-stripe-checkout";
...
function isValidFinnishAddress(address: string): boolean {
  ...
  const streetPattern = /[a-zA-ZäöåÄÖÅ][a-zA-ZäöåÄÖÅ\s\-'.]{1,}\s+\d+/i;
...
export function Cart() {
  ...
  const isDeliveryFeeApplied =
    cart.length > 0 &&
    orderType === "delivery" &&
    (!isWithinFreeDeliveryRadius || totalPrice <= DELIVERY_FEE_THRESHOLD);
  ...
  useEffect(() => {
    ...
    const paymentState = params.get("stripe")?.trim();
    const returnError = params.get("error")?.trim();

    if (!sessionId && !paymentState && !returnError) return;
    ...
    async function verifyReturn() {
      ...
      try {
        if (sessionId) {
          ...
          const data = await readApiJson<StripeVerifyResponse>(
            response,
  ...
  const validateDeliveryAddress = async (): Promise<DeliveryQuote | null> => {
    ...
    if (!isValidFinnishAddress(normalizedDeliveryAddress)) {
      setDeliveryAddressError(
        "Please enter a valid address with street name, number, and city (e.g., Kauppakatu 5, Turku).",
      );
      return null;
    }
  ...
  const handleProceedToPayment = async () => {
    ...
    if (!customerName.trim())
      errors.name = t("checkout.nameRequired") || "Name is required";
    if (!customerPhone.trim())
      errors.phone = t("checkout.phoneRequired") || "Phone is required";
    if (orderType === "delivery" && !normalizedDeliveryAddress) {
      errors.delivery =
        t("checkout.deliveryAddressRequired") || "Delivery address is required";
    }
    ...
    try {
      ...
      const response = await fetch(getApiUrl("/api/stripe/session"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...requestBody,
      ...
      const data = await readApiJson<StripeSessionResponse>(
        response,
            ...
            {cart.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-[var(--color-sumi)]/40 relative z-10">
                <ShoppingBag
                  ...
                  strokeWidth={1}
                />
                <p className="text-lg italic text-[var(--color-sumi)]/50 mb-2">
                  {t("order.emptyCart")}
                </p>
                <Link
                  to="/order"
                  className="mt-4 px-8 py-3 bg-[var(--color-sumi)] text-[var(--color-washi)] text-xs tracking-[0.2em] uppercase font-medium hover:bg-[var(--color-shu)] transition-colors"
                >
                  {t("order.addItems")}
                </Link>
              </div>
                      ...
                      <div className="flex-1 flex flex-col justify-between">
                        ...
                        <div className="flex justify-between items-center mt-4">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-medium text-[var(--color-sumi)] w-4 text-center">
                              {quantity}
                            </span>
                            <button
                          ...
                          <button
                            ...
                            className="text-[var(--color-sumi)]/40 hover:text-[var(--color-shu)] text-[10px] uppercase tracking-widest transition-colors font-medium"
                          >
                            {t("order.remove")}
                          </button>
                        </div>
                ...
                <div className="space-y-4 mb-6">
                  ...
                  <div className="flex gap-2 mb-4">
                    <button
                      ...
                      className={`flex-1 flex items-center justify-center gap-2 py-3 text-[10px] tracking-[0.15em] uppercase font-bold border transition-all ${orderType === "delivery" ? "border-[var(--color-shu)] bg-[var(--color-shu)]/10 text-[var(--color-shu)]" : "border-[var(--color-washi)]/20 text-[var(--color-washi)]/40 hover:border-[var(--color-washi)]/40"}`}
                    >
                      <Truck size={16} /> {t("order.delivery")}
                    </button>
                    <button
                      onClick={() => setOrderType("pickup")}
                      className={`flex-1 flex items-center justify-center gap-2 py-3 text-[10px] tracking-[0.15em] uppercase font-bold border transition-all ${orderType === "pickup" ? "border-[var(--color-shu)] bg-[var(--color-shu)]/10 text-[var(--color-shu)]" : "border-[var(--color-washi)]/20 text-[var(--color-washi)]/40 hover:border-[var(--color-washi)]/40"}`}
                    >
                  ...
                  {orderType === "delivery" && (
                    <div className="text-[10px] text-[var(--color-washi)]/50 leading-relaxed border-l-2 border-[var(--color-shu)] pl-2">
                      ...
                      within {FREE_DELIVERY_RADIUS_METERS / 1000}km for orders
                      over €{DELIVERY_FEE_THRESHOLD.toFixed(2)}.
                    </div>
                  )}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full bg-[var(--color-washi)]/[0.05] border text-[var(--color-washi)] px-4 py-3 pl-10 text-sm placeholder:text-[var(--color-washi)]/20 focus:outline-none focus:border-[var(--color-shu)] transition-colors ${formErrors.name ? "border-red-500/50" : "border-[var(--color-washi)]/20"}`}
                      />
                    </div>
                    {formErrors.name && (
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full bg-[var(--color-washi)]/[0.05] border text-[var(--color-washi)] px-4 py-3 pl-10 text-sm placeholder:text-[var(--color-washi)]/20 focus:outline-none focus:border-[var(--color-shu)] transition-colors ${formErrors.phone ? "border-red-500/50" : "border-[var(--color-washi)]/20"}`}
                      />
                    </div>
                    {formErrors.phone && (
                      <p className="mt-1 text-[10px] text-red-400 flex items-center gap-1">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-end justify-between">
                        ...
                        <button
                          ...
                          className="text-[10px] tracking-[0.15em] uppercase font-bold text-[var(--color-shu)] hover:text-[#a02020] transition-colors disabled:opacity-30 mb-0.5"
                        >
                          {isCheckingDelivery
                      ...
                      <textarea
                        ...
                        className={`w-full bg-[var(--color-washi)]/[0.05] border text-[var(--color-washi)] px-4 py-3 text-sm placeholder:text-[var(--color-washi)]/20 focus:outline-none focus:border-[var(--color-shu)] transition-colors resize-none h-24 ${formErrors.delivery || deliveryAddressError ? "border-red-500/50" : "border-[var(--color-washi)]/20"}`}
                      />
